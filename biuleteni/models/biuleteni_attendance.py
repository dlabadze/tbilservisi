...
class BiuleteniAttendance(models.Model):
    _name = 'biuleteni.attendance'
    _description = 'დასწრების დეტალები (ბიულეტენი)'

    biuleteni_det_id = fields.Many2one(
        'biuleteni_det',
        string='ბიულეტენი დეტალი',
        ondelete='cascade'
    )
    ...
    @api.depends('biuleteni_det_id')
    def _compute_display_fields(self):
        for rec in self:
            det = rec.biuleteni_det_id
            rec.month = det.month or ''
            rec.employee_id = det.biuleteni_id.employee_id if det and det.biuleteni_id else False
