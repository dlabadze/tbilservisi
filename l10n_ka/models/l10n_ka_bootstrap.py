...
class L10nKaBootstrap(models.AbstractModel):
    ...
    def _register_hook(self):
        ...
        self._sync_key_translations(env)
    ...
    @staticmethod
    def _sync_key_translations(env):
        term_map = {
            "Delete": "წაშლა",
            "Plan": "გეგმა",
            "Due Date": "ვადა",
            "To-Do": "შესასრულებელი",
            "e.g. Discuss proposal": "მაგ.: განიხილეთ წინადადება",
            "Log a note...": "შეიტანეთ შენიშვნა...",
            "This Month": "ეს თვე",
            "Worked hours this month": "ამ თვეში ნამუშევარი საათები",
            "Sync Emp and Contact": "თანამშრომლისა და კონტაქტის სინქრონიზაცია",
            "Send HR Documents Access Link": "HR დოკუმენტებზე წვდომის ბმულის გაგზავნა",
            "<b>Congratulations!</b> May I recommend you to setup an <a href=\"%s\">onboarding plan?</a>": "<b>გილოცავთ!</b> გირჩევთ, შექმნათ <a href=\"%s\">ადაპტაციის გეგმა</a>",
        }

        translations = env["ir.translation"].sudo()
        for lang in ("ka_GE", "ka"):
            for src, value in term_map.items():
                records = translations.search([
                    ("lang", "=", lang),
                    ("src", "=", src),
                ])
                if records:
                    records.write({"value": value, "state": "translated"})
