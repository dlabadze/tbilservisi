...
class AccountMove(models.Model):
    ...
    def generate_goods_list_xml_1(self, line):
        # If the line has a product, only process service-type products.
        # Non-service products are handled via the waybill upload (generate_goods_list_xml).
        if line.product_id:
            if line.product_id.type != 'service':
                _logger.info(f'Skipping non-service product line: {line.name}')
                return None
            # Service product: fall through and use მომსახურება as the unit
        elif line.unit_id:
            # No product but has a physical unit — skip (not a service line)
            _logger.info(f'Skipping line with unit_id (no product): {line.name}')
            return None
