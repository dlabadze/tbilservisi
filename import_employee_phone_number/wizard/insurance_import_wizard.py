...
class InsuranceImportWizard(models.TransientModel):
    _name = 'insurance.import.wizard'
    _description = 'Insurance Import Wizard'
    ...
    def action_import(self):
        ...
        try:
            ...
            # Columns: 6 = identification_id (0-based index 5), 16 = sum_premia (0-based index 15)
            data = []
            for row_idx in range(self.start_row - 1, min(self.end_row, len(sheet_data))):
                row = sheet_data[row_idx]
                if len(row) < 16:
                    continue
                identification_id = row[5]
                sum_premia = row[15]
            ...
            for d in data:
                ...
                if not employee:
                    missed_rows.append({
                        ...
                        'sum_premia': d['sum_premia'],
                        'error': _('Employee not found (identification_id)'),
                    })
                    continue
                employee = employee[:1]
                write_vals = {}
                if 'x_studio_insurance' in employee._fields:
                    write_vals['x_studio_insurance'] = d['sum_premia']
                if write_vals:
                    employee.write(write_vals)
                updated_count += 1

            if missed_rows:
                # First show notification how many employees updated
                notification = {
                ...
                # Generate missed_rows Excel and return download action (optionally chained after notification)
                ...
                # Show notification then trigger download via next
    ...
    def _generate_missed_rows_excel(self, missed_rows):
        ...
        headers = [_('Excel Row'), _('Identification ID'), _('Sum Premia'), _('Error')]
        ...
        for row_idx, row_data in enumerate(missed_rows, 2):
            ...
            ws.cell(row=row_idx, column=3, value=row_data.get('sum_premia'))
            ws.cell(row=row_idx, column=4, value=row_data.get('error', ''))
        buffer = BytesIO()
        wb.save(buffer)
        buffer.seek(0)
        attachment = self.env['ir.attachment'].create({
            'name': _('insurance_import_missed_rows.xlsx'),
